...
export default function CartScreen() {
    ...
    const handleOpenCart = (cartId: string) => {
        router.push({ pathname: '/cart_detail', params: { cartId } });
    };
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            <View style={styles.container}>
                ...
                <TouchableOpacity style={styles.createCartButton} onPress={handleCreateCart}>
                    ...
                    {!isUnlocked && (
                        <View style={styles.progressSection}>
                            <View style={styles.progressLabelsRow}>
                                ...
                                <Text style={[styles.progressLabel, styles.progressLabel10]}>10</Text>
                                <Text style={[styles.progressLabel, styles.progressLabel20]}>20</Text>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                    {carts.length === 0 ? (
                        ...
                        carts.map((cart) => {
                            const itemsCount = cart.products.reduce((sum, p) => sum + p.quantity, 0);
                            const currentTotal = cart.products.reduce((sum, p) => sum + p.price * p.quantity, 0);
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <View style={styles.cartInfo}>
                                            ...
                                            <Text style={styles.subText}>Budget: €{cart.budget}</Text>
                                            <Text style={styles.subText}>In cart: €{currentTotal.toFixed(2)}</Text>
                                            <Text style={styles.subText}>{itemsCount} items</Text>
...
const styles = StyleSheet.create({
    ...
    createCartButton: {
        height: 54, borderRadius: 16, backgroundColor: '#111',
        flexDirection: 'row', justifyContent: 'center', alignItems: 'center',
        gap: 8, marginBottom: 14,
    },
    createCartButtonText: { fontSize: 15, fontWeight: '700', color: '#fff' },
    counterText: { fontSize: 13, color: '#6d6d6d', marginBottom: 14 },
    upgradeBanner: {
        borderRadius: 18, overflow: 'hidden',
        paddingHorizontal: 16, paddingVertical: 14, marginBottom: 18,
    },
    upgradeBannerImage: { borderRadius: 18 },
    upgradeTopRow: {
        flexDirection: 'row', alignItems: 'flex-start',
        justifyContent: 'space-between', gap: 10,
    },
    upgradeTitle: { fontSize: 20, fontWeight: '800', color: '#111', marginBottom: 6 },
    upgradeText: { fontSize: 13, lineHeight: 18, color: '#111' },
    upgradeButton: {
    ...
    progressLabelsRow: {
        position: 'relative', height: 24, marginBottom: 3, marginHorizontal: 12,
    },
    progressLabel: { fontSize: 15, fontWeight: '700', color: '#111' },
    progressLabelStart: { position: 'absolute', left: 0 },
    progressLabel10: { position: 'absolute', left: '33.33%', marginLeft: -12 },
    progressLabel20: { position: 'absolute', left: '66.66%', marginLeft: -12 },
    progressLabelEnd: { position: 'absolute', left: '100%', marginLeft: -30 },
    unlockLabelWrapper: { flexDirection: 'row', alignItems: 'center', gap: 4 },
    progressTrackWrapper: {
        position: 'relative', height: 34, justifyContent: 'center', marginHorizontal: 12,
    },
    progressTrack: { height: 8, borderRadius: 999, backgroundColor: '#c9c9c9' },
    progressFill: {
        position: 'absolute', left: 0, height: 8, borderRadius: 999, backgroundColor: '#111',
    },
    progressMarker: { position: 'absolute', marginLeft: -11, bottom: -8 },
    tickMark: {
        position: 'absolute', width: 3, height: 14, backgroundColor: '#111',
        top: 10, marginLeft: -1.5, borderRadius: 2,
    },
    tick10: { left: '33.33%' },
    tick20: { left: '66.66%' },
    tick30: { left: '100%' },
    ...
    cartCard: {
        flexDirection: 'row', alignItems: 'center', backgroundColor: '#ededed',
    ...
    swipeButton: {
        width: 86, borderRadius: 18, justifyContent: 'center',
        alignItems: 'center', gap: 6, marginLeft: 8,
    },
    editSwipeButton: { backgroundColor: '#dedede' },
    deleteSwipeButton: { backgroundColor: '#111' },
    swipeButtonText: { fontSize: 13, fontWeight: '700', color: '#111' },
    deleteSwipeButtonText: { fontSize: 13, fontWeight: '700', color: '#fff' },
    emptyText: {
        textAlign: 'center', marginTop: 60, fontSize: 16,
