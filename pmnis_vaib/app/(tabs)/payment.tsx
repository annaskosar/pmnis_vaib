...
export default function PaymentScreen() {
    ...
    // Debug – vymaž po oprave
    console.log('Payment screen cartId param:', cartId);
    console.log('Parsed cartId:', parsedCartId);
    console.log('Found cart:', getCartById(parsedCartId));
    ...
    // Vždy zavolaj getCartById – aj počas loadingu
    ...
    // Počas loadingu zobraz loading
    if (isCartLoading) {
        return (
            <SafeAreaView style={styles.safeArea}>
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    <Text style={{ fontSize: 18, fontWeight: '600', color: '#111' }}>Loading...</Text>
                </View>
            </SafeAreaView>
        );
    }

    // Cart not found — AŽ po dokončení loadingu
    if (!cart) {
        return (
            <SafeAreaView style={styles.safeArea}>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
                        ...
                        <View style={styles.card}>
                            ...
                            <View style={styles.summaryRow}>
                                ...
                                <Text style={styles.summaryPrice}>{deliveryPrice === 0 ? 'Free' : `€${deliveryPrice.toFixed(2)}`}</Text>
                            </View>
                            <View style={styles.summaryRow}>
                        ...
                        <View style={styles.card}>
                            {deliveryOptions.map(option => (
                                <TouchableOpacity key={option.id} style={styles.deliveryOption} onPress={() => setDeliveryMethod(option.id as any)}>
                                    ...
                                    <Text style={styles.deliveryPrice}>{option.price === 0 ? 'Free' : `€${option.price.toFixed(2)}`}</Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                            ...
                            <>
                                ...
                                <View style={styles.card}>
                                    <View style={styles.inputWrapper}>
                                        ...
                                        <TextInput style={styles.input} placeholder="Street and number" placeholderTextColor="#999" value={street} onChangeText={setStreet} />
                                    </View>
                                    <View style={styles.inputRow}>
                                        ...
                                        <View style={[styles.inputWrapper, { width: 100 }]}>
                                            <TextInput style={styles.input} placeholder="ZIP" placeholderTextColor="#999" value={zip} onChangeText={setZip} keyboardType="numeric" />
                                        </View>
                                    </View>
                                    <View style={styles.inputWrapper}>
                        ...
                        <View style={styles.card}>
                            <View style={styles.inputWrapper}>
                                ...
                                <TextInput style={styles.input} placeholder="Card number" placeholderTextColor="#999" value={cardNumber} onChangeText={text => setCardNumber(formatCardNumber(text))} keyboardType="numeric" maxLength={19} />
                            </View>
                            <View style={styles.inputWrapper}>
                                <Feather name="user" size={16} color="#999" />
                                <TextInput style={styles.input} placeholder="Cardholder name" placeholderTextColor="#999" value={cardName} onChangeText={setCardName} autoCapitalize="words" />
                            </View>
                            <View style={styles.inputRow}>
                                <View style={[styles.inputWrapper, { flex: 1 }]}>
                                    <Feather name="calendar" size={16} color="#999" />
                                    <TextInput style={styles.input} placeholder="MM/YY" placeholderTextColor="#999" value={expiry} onChangeText={text => setExpiry(formatExpiry(text))} keyboardType="numeric" maxLength={5} />
                                </View>
                                <View style={[styles.inputWrapper, { width: 120 }]}>
                                    <Feather name="lock" size={16} color="#999" />
                                    <TextInput style={styles.input} placeholder="CVV" placeholderTextColor="#999" value={cvv} onChangeText={text => setCvv(text.replace(/\D/g, '').slice(0, 4))} keyboardType="numeric" secureTextEntry={!showCvv} maxLength={4} />
...
const styles = StyleSheet.create({
    ...
    backButton: { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f3f3f3' },
    headerTitle: { fontSize: 20, fontWeight: '700', color: '#111' },
    scrollContent: { paddingBottom: 40 },
    sectionTitle: { fontSize: 16, fontWeight: '700', color: '#111', marginBottom: 10, marginTop: 16 },
    card: { backgroundColor: '#ededed', borderRadius: 18, padding: 14, borderWidth: 1, borderColor: '#d6d6d6', gap: 10 },
    ...
    deliveryOption: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 4 },
    deliveryLeft: { flexDirection: 'row', alignItems: 'center', gap: 12 },
    radioOuter: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: '#aaa', justifyContent: 'center', alignItems: 'center' },
    ...
    inputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f3f3f3', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 13, gap: 10 },
    input: { flex: 1, fontSize: 15, color: '#111' },
    inputRow: { flexDirection: 'row', gap: 10 },
    payButton: { backgroundColor: '#111', borderRadius: 20, paddingVertical: 16, marginTop: 24, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
